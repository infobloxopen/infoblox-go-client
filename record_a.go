...
func (objMgr *ObjectManager) CreateARecord(recA RecordA) (*RecordA, error) {
	...
	recordA.Ref = ref
	//recordA.returnFields = []string{"name","extattrs","view","zone"}
...
func (objMgr *ObjectManager) DeleteARecord(recA RecordA) (string, error) {
	var res []RecordA
	recordName := NewRecordA(RecordA{})
	if len(recA.Ref) > 0 {
		return  objMgr.connector.DeleteObject(recA.Ref)

	} else if len (recA.Name) > 0 {
		recordName = NewRecordA(RecordA{Name: recA.Name, View: recA.View})

	} else if len (recA.Ipv4Addr) > 0 {
		recordName = NewRecordA(RecordA{Ipv4Addr: recA.Ipv4Addr})
...
func (objMgr *ObjectManager) UpdateARecord(recA RecordA) (*RecordA, error) {
	...
	res.Ref= reference
	//err = objMgr.connector.GetObject(&recordA, res.Ref, &res)
	return &res, err
}
