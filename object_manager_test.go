...
func (c *fakeConnector) GetObject(obj IBObject, ref string, qp *QueryParams, res interface{}) (err error) {
	if reflect.TypeOf(c.getObjectObj).Kind() == reflect.Map {
		...
		case *DtcServer:
			Expect(obj).To(Equal(c.getObjectObj.(map[string]interface{})["DtcServer"].(*DtcServer)))
			Expect(qp).To(Equal(c.getObjectQueryParams.(map[string]*QueryParams)["DtcServer"]))
			*res.(*[]DtcServer) = c.resultObject.(map[string]interface{})["DtcServer"].([]DtcServer)
		case *DtcMonitorHttp:
			Expect(obj).To(Equal(c.getObjectObj.(map[string]interface{})["DtcMonitor"].(*DtcMonitorHttp)))
			Expect(qp).To(Equal(c.getObjectQueryParams.(map[string]*QueryParams)["DtcMonitor"]))
