...
func (objMgr *ObjectManager) CreatePTRRecord(recPTR RecordPTR) (*RecordPTR, error) {
	...
	eas := objMgr.extendEA(recPTR.Ea)

	recordPTR := NewRecordPTR(RecordPTR{
		View:     recPTR.View,
		PtrdName: recPTR.PtrdName,
		Ea:       eas})
	...
	recordPTR.Ref = ref
	recordPTR.returnFields = []string{"ptrdname","view"}
...
func (objMgr *ObjectManager) DeletePTRRecord(recPTR RecordPTR) (string, error) {
	...
	} else if len (recPTR.Name) > 0 {
		recordName = NewRecordPTR(RecordPTR{Name: recPTR.Name})

	} else if len (recPTR.PtrdName) > 0 {
		recordName = NewRecordPTR(RecordPTR{PtrdName: recPTR.PtrdName})
